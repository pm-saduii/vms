...
## ✅ Feature — Houses ซอย column + Admin Dashboard Charts

### ข้อ 1: ตารางบ้านเพิ่มคอลัมน์ซอย
- `<th>ซอย</th>` เพิ่มหลัง บ้านเลขที่
- แต่ละ row: บ้านเลขที่ | `<span class="tag">ซอย X</span>` | เจ้าของ | ...
- เรียงข้อมูลตามซอย → บ้านเลขที่ (มีอยู่แล้ว ยืนยัน)
- colspan จาก 7 → 8

### ข้อ 2.1: Dashboard คำขอแก้ไขล่าสุด
- `_renderDashReqs`: lookup `window._housesAllCache` → แสดง `บ้านเลขที่ + ซอย` แทน `house_id`
- แปล `target_type`: house_info → แก้ไขข้อมูลบ้าน, vehicle_add → ขอเพิ่มรถ, vehicle_edit → แก้ไขข้อมูลรถ

### ข้อ 2.2: Vehicle Cards (Row 6)
- 4 cards: รถยนต์ / รถจักรยานยนต์ / รถตู้-กระบะ / อื่นๆ
- `loadAdminDashboard` fetch `getVehicles` → นับตามประเภท → fill `dash-veh-*`

### ข้อ 2.3-2.4: Bar Charts รายเดือน (Real data)
- Chart 3: การแจ้งกระทำผิด รายเดือน → data จาก `violations.created_at`
- Chart 4: แจ้งปัญหา + แก้ไขแล้ว → data จาก `issues.created_at` + `issues.updated_at`
- ทั้ง 2 charts: เลือกปีได้ + เปลี่ยนสีได้ (3 สีตัวเลือก)
- `Chart.js` โหลด lazy (เฉพาะตอนเปิด dashboard)

### ข้อ 2.5-2.6: Fee Charts (Mock data)
- Chart 5: บ้านค้างชำระ vs ชำระแล้ว แยกครึ่งปี (mock)
- Chart 6: ยอดเก็บได้ vs เป้าหมาย แยกครึ่งปี (mock)
- ทั้ง 2 charts: เลือกปีได้ (2568/2567/2566)

### ไม่ต้อง deploy GAS

---

## ✅ Feature — หน้าแจ้งปัญหาลูกบ้าน Redesign

### 1. แก้ข้อความ "แจ้ปัญหา" → "แจ้งปัญหา"
- PT breadcrumb map `'res-issue'` ที่ encode ผิดเป็น `\u0e41\u0e08\u0e49\u0e1b\u0e31\u0e0d\u0e2b\u0e32`

### 2. p-res-issue HTML ใหม่
- Page header: ปุ่ม refresh + ปุ่ม "แจ้งปัญหาใหม่" (เรียก `openResNewIssueModal`)
- 2 tabs: 🔴 รอดำเนินการ (count) / ✅ เสร็จสิ้น
- ID ใช้ `res-tab-iss-open/done` ไม่ชนกับ admin issues tabs

### 3. Modal m-newissue ใหม่
- `md-sub`: populate บ้านจริงจาก `APP.house` (ไม่ hardcode "10/1")
- ประเภทปัญหา: `id="new-issue-cat"` (เดิมไม่มี id → `doSubmitIssue` crash)
- ลำดับ: ประเภท → หัวข้อ → รายละเอียด → รูปภาพ
- Image upload: resize ≤150KB, preview + ลบได้, max 3 รูป

### 4. loadResIssuePage ใหม่
- แยก open/done list
- แต่ละ card: icon ประเภท + title + status badge + category + วันที่ + desc + admin_note (highlighted) + รูปภาพ
- ปุ่ม "⭐ ให้คะแนน" เมื่อ resolved แต่ยังไม่ rating

### 5. doSubmitIssue ใหม่
- สร้างปัญหาก่อน → upload รูป (filename `ISS-{issue_id}_N.jpg`) → update image_urls
- showLoader: 'กำลังส่งแจ้งปัญหา...'
- reset _issImages หลังส่ง

### 6. Helper functions ใหม่
- `onIssImageSelect()`, `_renderIssImgPreviews()` — image upload + preview
- `openResNewIssueModal()` — reset form + populate house sub + openM
- `stabResIssue(tab)` — toggle tabs

---

## ✅ Fix — หน้าจัดการปัญหา + Dashboard Charts

### 1. Reload ผิดที่
- แก้ `btn-refresh` จากอยู่ใน `ph-sub` → ย้ายไปอยู่ใน `ph-acts` (มุมขวาถูกต้อง)

### 2. เปิดหน้ามา Load รายการรอดำเนินการทันที
- ยืนยัน: `loadPageData` เรียก `loadIssuesPage()` ตอน navigate มา `admin-issues`
- Tab default = `ai-open` (รอดำเนินการ) แสดงทันที

### 3. Update ปัญหาแนบรูปได้
- `m-update-issue` modal: เพิ่ม image upload section ใช้ prefix `iss-admin-` (แยกจาก resident `iss-`)
- แสดงรูปเดิม + ปุ่ม × ลบ → `_issDeleteExistingImg(idx, fileId)`
- `doUpdateIssueStatus`: upload รูปใหม่ → merge กับรูปเดิม → ส่ง `image_urls`
- **GAS `Issues.updateStatus`**: เพิ่มรองรับ `image_urls` parameter

### 4. Status labels ภาษาไทย
- `_issStatusTH`: pending→รอดำเนินการ, in_progress→กำลังดำเนินการ, resolved→เสร็จสิ้น, closed→ปิด
- Tab labels: กำลังดำเนิน → กำลังดำเนินการ, เสร็จสิ้น → เสร็จสิ้น/ปิด

### 5. ช่องหมายเหตุ reset ทุกครั้ง
- `openUpdateIssue`: `noteEl.value = ''` ก่อนเปิด modal ทุกครั้ง
- populate เฉพาะ `status` จากค่าเดิม (ไม่ populate `admin_note`)

### 6. ปุ่มให้ดาวลูกบ้าน
- `loadResIssuePage` มีปุ่ม "⭐ ให้คะแนน" สำหรับ status='resolved' อยู่แล้ว (บรรทัด 4461)
- เพิ่ม `_renderResIssueList` helper สำหรับ future use

### Dashboard Charts ว่างตอนเปิด
- **Root cause**: `renderAdminDashboardFromBoot` ไม่เรียก `dashRenderAllCharts` → chart ว่างหน้าแรก
- **Fix**: เพิ่ม chart data calculation จาก `boot.violations` + `boot.issues` → `window._dashVioData/IssData` → เรียก `dashRenderAllCharts()` ใน boot render

### ⚠️ Deploy GAS: Fees_Issues_Violations.gs

---

## ✅ Fix — จัดการปัญหา (Admin+Resident) + Dashboard Charts

### Admin: หน้าจัดการปัญหา
1. **หัวโปรแกรม** — h1 + breadcrumb: `แจ้ปัญหา — จัดการ` → `จัดการปัญหา`
2. **Tab auto-load** — tab `ai-open` เปิดค่า on ตั้งแต่ต้น + `loadIssuesPage` เรียกผ่าน `loadPageData` ทันทีที่ navigate
3. **ปุ่มดูปัญหา** — เพิ่ม modal `m-view-issue` แสดง: title, สถานะ, บ้าน, หมวด, วันที่, คำอธิบาย, หมายเหตุนิติ, รูปลูกบ้าน — ปุ่ม "✏️ อัปเดตสถานะ" ใน modal เรียก `openUpdateFromView()`
4. **หมายเหตุห้ามล้าง** — `openUpdateIssue` populate `iss.admin_note` เดิมลงช่องแทนที่จะล้าง (กรอกเพิ่มเติมได้)
5. **ไม่แสดงดาว** — `_renderIssueList` (admin) ไม่มี rating HTML เลย (เฉพาะ resident มี)

### Resident: หน้าแจ้งปัญหา
1. **แสดงรูป** — `renderIssCard` render `image_urls` เป็น thumbnail row ครบแล้ว
2. **ปุ่ม Cancel** — แสดงเมื่อ status=pending/in_progress → `doCancelIssue(issue_id)` → ยืนยัน Swal → `api('cancelIssue')` → status=`cancelled`
   - Tab "เสร็จสิ้น" → "เสร็จสิ้น/ยกเลิกโดยลูกบ้าน"
   - filter done: resolved|closed|cancelled
   - status label: `cancelled` → "ยกเลิกโดยลูกบ้าน"
3. **ให้ดาวแล้วปิดอัตโนมัติ** — `doSubmitRating` หลัง `rateIssue` → เรียก `cancelIssue({status:'closed'})` → issue ปิดทันที ไม่ต้องให้นิติกด

### GAS: Fees_Issues_Violations.gs
- `Issues.cancel(body, user)` — ลูกบ้านยกเลิกได้เฉพาะ house ตัวเอง, admin ยกเลิกได้ทุกรายการ
- รองรับ `body.status` override (ใช้เปลี่ยนเป็น `closed` เมื่อให้ดาว)
- export เพิ่ม `cancel`

### GAS: Code.gs
- เพิ่ม route `'cancelIssue': () => Issues.cancel(body, user)`

### Dashboard Charts — ไม่ต้อง Reload
- **Root Cause:** `dashRenderAllCharts` lazy-load `<script>` element ทุกครั้งที่เรียก → race condition → script โหลดซ้ำ หรือ `onload` fire ช้ากว่า data
- **Fix:** โหลด Chart.js ใน `<head>` ตั้งแต่ต้น `<script src="https://cdnjs.cloudflare.com/.../chart.umd.js">`
- `dashRenderAllCharts` เปลี่ยนเป็น fallback `setTimeout(..., 300)` แทน dynamic script inject

### ⚠️ Deploy GAS: Code.gs + Fees_Issues_Violations.gs
